...
object SMTRange {
  ...
  def toConstraintsSingle(v: Variable, i: Interval): Expr = {
  ...
  def cleanSubIntervals(intervals: ParSeq[Map[Identifier, Interval]], precond: Expr):ParSeq[Map[Identifier, Interval]] = {

    intervals.tasksupport = new ForkJoinTaskSupport(new scala.concurrent.forkjoin.ForkJoinPool(Runtime.getRuntime().availableProcessors()))

    val tempIntervals = intervals.par.filter{ (interval) =>
      //println("OK")
      var solverQuery = precond
      for (t <- interval)
        solverQuery = And(toConstraintsSingle(Variable(t._1),t._2),solverQuery)
      val res = Solver.checkSat(solverQuery)
    ...
    tempIntervals.tasksupport = new ForkJoinTaskSupport(new scala.concurrent.forkjoin.ForkJoinPool(Runtime.getRuntime().availableProcessors()))
