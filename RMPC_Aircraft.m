function RMPC_Aircraft(delta,epss)
    ...
    KK=place(A,B,[-5,-3,-1,-2]);%work for .1
    %KK=place(A,B,[-.5,-.3,.1,-2]);
    A=A-B*KK;
    Ts=.1;
    N=1;
    W_x_bounds = [-delta,delta;-delta,delta]*1;
    Y_x_Limit_orig = [5;10]; % 
    U_x_bounds = [-25,25;-25,25]; %
    Q = 5*diag(ones(size(C,1),1)); 
    R = 1*diag(ones(size(B,2),1)); 
    Y_ref = [0;0]; 
    Q_L=1;
    R_L=1;
    RANGE=[5,10,5,10];
    ...
    [H,K,F,G,Nc] = GetRobustMPC_Matrices(sol_x_mp{1});    Nx = length(add_usys_d.matrices.A);
    %%
    close all
...
%                                 end
                                 ...
                                 %%vertex computation which works for 2D
%                                 H_intersec(1,:)=hh(1,1:size(A,1));
...
%                                 for v_intersec_ind=1:1:size(ver_intersec,1) 
%                                     for cornerr_ind=1:1:size(binary_mat,1)
%                                         for epss_dim=1:1:size(A,1)
%                                             if binary_mat(cornerr_ind,epss_dim)==1
%                                                 line_corner_set(cornerr_ind+(v_intersec_ind-1)*size(binary_mat,1),epss_dim)=ver_intersec(v_intersec_ind,epss_dim)+eps;
%                                             else
%                                                 line_corner_set(cornerr_ind+(v_intersec_ind-1)*size(binary_mat,1),epss_dim)=ver_intersec(v_intersec_ind,epss_dim)-eps;
%                                             end
%                                         end
...
%     end
        ...
        for line_ind=1:1:size(ineq_set_left,1)
            ...
            for dim=1:1:size(A,1)
                if dim==1
                    ...
                    if LINE_RANGE(line_ind_glob,size(A,1)+1+1)<-RANGE(dim)
                        LINE_RANGE(line_ind_glob,size(A,1)+1+1)=-RANGE(dim);
                    elseif LINE_RANGE(line_ind_glob,size(A,1)+1+1)>RANGE(dim)
                        LINE_RANGE(line_ind_glob,size(A,1)+1+1)=RANGE(dim);
                    end
                    if LINE_RANGE(line_ind_glob,size(A,1)+1+2)>RANGE(dim)
                        LINE_RANGE(line_ind_glob,size(A,1)+1+2)=RANGE(dim);
                    elseif LINE_RANGE(line_ind_glob,size(A,1)+1+2)<-RANGE(dim)
                        LINE_RANGE(line_ind_glob,size(A,1)+1+2)=-RANGE(dim);
                    end
                    pointer=4+size(A,1);
                else
                    LINE_RANGE(line_ind_glob,pointer:pointer+1)=[min_line(line_ind,dim),max_line(line_ind,dim)];
                    if LINE_RANGE(line_ind_glob,pointer)<-RANGE(dim)
                        LINE_RANGE(line_ind_glob,pointer)=-RANGE(dim);
                    elseif LINE_RANGE(line_ind_glob,pointer)>RANGE(dim)
                        LINE_RANGE(line_ind_glob,pointer)=RANGE(dim);
                    end
                    if LINE_RANGE(line_ind_glob,pointer+1)>RANGE(dim)
                        LINE_RANGE(line_ind_glob,pointer+1)=RANGE(dim);
                    elseif LINE_RANGE(line_ind_glob,pointer+1)<-RANGE(dim)
                        LINE_RANGE(line_ind_glob,pointer+1)=-RANGE(dim);
                    end
                    pointer=pointer+2;
                end
    ...
    printMatrix(Nc_acc,'region_pointer')
